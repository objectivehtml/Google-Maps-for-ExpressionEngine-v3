...
class Channel_data_utility extends Channel_data_lib {
	...
	{
		$fields = $this->get_channel_fields($channel_id);
		...
		$post   = array(
			...
			'entry_date'      => isset($data->{($prefix.'entry_date')}) ? $data->{($prefix.'entry_date')} : $this->EE->localize->now,
			'expiration_date' => isset($data->{($prefix.'expiration_date')}) ? $data->{($prefix.'expiration_date')} : 0,
