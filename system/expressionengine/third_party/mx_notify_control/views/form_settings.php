...
<?php
		...
		if (isset($settings['row_order'])) {
			...
			{
			$out_result  .= table_notify($input_prefix, $row_ids,$row_id, $template_list,  $settings, $member_groups, $channel_data, $statuses);
			$row_ids	+=	1;
			}
...
<?php
			...
			.'template = "'. str_replace('"','\"',str_replace(array("\r", "\r\n", "\n\r", "\n"), "", table_notify($input_prefix, "{row}","{row}", $template_list, null, $member_groups, $channel_data, $statuses))).'";
			 index_row = '.$row_ids.';
...
function table_notify($input_prefix, $row_ids,$row_id, $template_list, $settings, $member_groups,$channel_data, $statuses) {
